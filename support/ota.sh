...
WEBSITE_URL="https://userariii.github.io/"
NEWS_URL="https://t.me/SD720G_repo"
JSON_FMT='{\n\t"error":false,\n\t"filename":"%s",\n\t"datetime":%s,\n\t"size":%s,\n\t"url":"%s",\n\t"filehash":"%s",\n\t"version":"%s",\n\t"status":"%s",\n\t"pexv":"%s",\n\t"id":"%s",\n\t"tg_username":"%s",\n\t"device_name":"%s",\n\t"device":"%s",\n\t"xda_thread":"%s",\n\t"maintainers": [{\n\t\t"main_maintainer":false,\n\t\t"github_username":"%s",\n\t\t"name":"%s"\n\t}],\n\t"donate_url":"%s",\n\t"website_url":"%s",\n\t"news_url":"%s",\n\t"forum_url":"%s"\n}'

printf "$JSON_FMT" "$FILENAME" "$DATETIME" "$SIZE" "$URL" "$FILEHASH" "$VERSION" "$STATUS" "$PEXV" "$ID" "$TG_USERNAME" "$DEVICE_NAME" "$DEVICE" "$XDA_THREAD" "$GHUN" "$NAME" "$DONATE_URL" "$WEBSITE_URL" "$NEWS_URL" "$XDA_THREAD" > OTA/pixelexperience/tiramisu/builds/$DEVICE.json
echo OTA/pixelexperience/tiramisu/builds/$DEVICE.json file created
...
if [ -d "OTA/pixelexperience/tiramisu/changelogs/$DEVICE" ]
then
printf "$CHANGELOG" > OTA/pixelexperience/tiramisu/changelogs/$DEVICE/$FILENAME.txt
echo OTA/pixelexperience/tiramisu/changelogs/$DEVICE/$FILENAME.txt file created "(make sure to update the changelog file)"
else
mkdir -p OTA/pixelexperience/tiramisu/changelogs/$DEVICE
